...
SKAT_res <- read.csv(file= paste(input_file_direc,'Discovery_TCGA_SKATO_random_randomized_geneset.txt',sep=''),head=TRUE,sep ='\t',stringsAsFactors=FALSE) %>%
...
write.table(FDR_calculated,
            file= paste(output_file_direc,'TCGA_empirical_FDRs_random_geneset.txt',sep=''),
...
##upload SKAT-O results from randomization
#remove models which will be removed based on inflation factor
SKAT_res <- read.csv(file= paste(input_file_direc,'Validation_PCAWG_Hartwig_SKATO_random_randomized_geneset.txt',sep=''),head=TRUE,sep ='\t',stringsAsFactors=FALSE) %>%
  dplyr::filter(!model_snps_cancer_pheno %in% models_discard$model_snps_cancer_pheno)
...
write.table(FDR_calculated,
            file= paste(output_file_direc,'PCAWG_Hartwig_empirical_FDRs_random_geneset.txt',sep=''),
            quote=FALSE, sep='\t',row.names=FALSE,col.names = T)
