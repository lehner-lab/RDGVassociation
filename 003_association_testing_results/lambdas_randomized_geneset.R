---
  title: "Estimation of inflation factors for randomized geneset"
---
...
pdf(file= paste(output_figure_direc,table_name,'_lambdas_distribution_VAE_ICA_randomized_geneset','.pdf',sep=''),
    width= 8,
    height =10)  
...
models_removed <- lambdas_results_filtered %>%
  ...
  mutate(cancer_type=sub('Brain\nglioma\nmulti','Brain_glioma_multi',cancer_type)) %>%
  mutate(cancer_type=sub('Brain\nglioma\nlow','Brain_glioma_low',cancer_type))  %>%
  mutate(cancer_type=sub('Stomach\nEso','Stomach_Eso',cancer_type))  %>%
  mutate(cancer_type=sub('Colon\nRectum','Colon_Rectum',cancer_type))

write.table(models_removed,
            file= paste(output_file_direc,'models_removed_based_on_lambda_randomized_geneset.txt',sep=''),
