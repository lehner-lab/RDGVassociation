...
parser.add_argument('-v', '--dataset_test', 
                    help='test dataset, put in full name + direc')
...
test_df_input = np.array(test_df.drop(['sample_short'], axis=1))
#print(train_df_input.shape)
#print(test_df_input.shape)
...
history = vae.fit(train_df_input,
                  ...
                  validation_data=(test_df_input, None),
                  callbacks=[WarmUpCallback(beta, kappa)])

# evaluate final loss
training_loss= vae.evaluate(train_df_input)
validation_loss= vae.evaluate(test_df_input)
...
encoded_test_df = encoder.predict_on_batch(test_df_input)
encoded_test_df = pd.DataFrame(encoded_test_df, columns= range(1, latent_dim+1))

sum_node_activity = encoded_test_df.sum(axis=0).sort_values(ascending=False)
...
encoded_mean_test = encoder.predict_on_batch(test_df_input)

encoded_mean_train_df = pd.DataFrame(encoded_mean_train, columns= range(1, latent_dim+1))
encoded_mean_test_df = pd.DataFrame(encoded_mean_test, columns= range(1, latent_dim+1))

#add sample short info
train_df_newindex = train_df.copy()
test_df_newindex = test_df.copy()
...
test_df_newindex.index =  range(test_df_newindex.shape[0])
encoded_mean_test_df = encoded_mean_test_df.assign(sample_short= test_df_newindex.sample_short)

#combine training and test mean encodings
encoded_mean_dataset_df = pd.concat([encoded_mean_train_df,encoded_mean_test_df])
...
# encoding again
val_encoded = encoder.predict_on_batch(test_df_input)
...
validation_df= pd.DataFrame(test_df_input, columns=train_df.drop(['sample_short'], axis=1).columns)
...
output_parameter_model_run='./results/parameters_test'
...
output_df_direc = os.path.join(output_parameter_model_run, "VAE_mse_plus_KL_" + str(latent_dim) + "_components_" + 
                               ...
                               str(1) + "_beta_" + str(hidden_dim) + "_hiddenDim_" + str(depth) + "_depth" + ".txt")
#save output
